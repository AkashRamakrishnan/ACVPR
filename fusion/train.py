...
for epoch in range(num_epochs):
    train_loss = 0
    for i, (traj, sequences, labels) in enumerate(tqdm(train_dataloader)):
        traj = traj.float()
    ...
    with torch.no_grad():
        ...
        for val_traj, val_sequences, val_labels in tqdm(val_dataloader):
            val_traj = val_traj.float()
            val_sequences = val_sequences.float()
            val_labels = val_labels.long()
            val_traj.to(device)
            val_sequences.to(device)
            val_labels.to(device)
            val_outputs = model(val_sequences, val_traj)
