...
def plot_one_box_kpt(x, text_id, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2,
                     orig_shape=None,predicted_cx=None, predicted_cy=None):
    ...
    cx = (c1[0] + c2[0]) // 2
    cy = (c1[1] + c2[1]) // 2
    cv2.circle(im, (cx, cy), 3, (0, 255, 0), -1)  # Plot center of gravity as a green circle
    if label:
    ...
    if text_id:
        tf_id = max(tl - 1, 1)  # font thickness for ID
        t_size_id = cv2.getTextSize(text_id, 0, fontScale=tl / 6, thickness=tf_id)[0]
        c2_id = c1[0] + t_size_id[0], c1[1] - t_size_id[1] - 15
        cv2.rectangle(im, c1, c2_id, color, -1, cv2.LINE_AA)  # filled for ID
        cv2.putText(im, text_id, (c1[0], c1[1] - 12), 0, tl / 6, [255, 255, 255], thickness=tf_id // 2,
                    lineType=cv2.LINE_AA)
        if predicted_cx is not None and predicted_cy is not None:
            predicted_cx = int(predicted_cx)
            predicted_cy = int(predicted_cy)
            cv2.circle(im, (predicted_cx, predicted_cy), 3, (0, 255, 0), -1) 
       
    if kpt_label:
        plot_skeleton_kpts(im, kpts, steps, orig_shape=orig_shape)

def plot_one_box_cogbox(x, im, line_thickness=3, steps=2,color=None):
